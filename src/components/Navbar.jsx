...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { name: "Post Gig", path: "/post-task", icon: "add_circle" },
    { name: "My Gigs", path: "/tasks", icon: "list_alt" },
  ];
    ...
    <>
      ...
      <aside className="fixed left-0 top-0 h-full flex flex-col p-6 w-60 border-r-[3px] border-on-surface bg-surface shadow-[4px_4px_0px_0px_rgba(48,52,44,1)] z-50 hidden md:flex">
        <div className="mb-10 cursor-pointer" onClick={() => navigate("/")}>
          ...
          <p className="text-[9px] font-bold uppercase tracking-widest text-primary opacity-70">Aggressive Curator</p>
        </div>
            ...
            <Link
              ...
              className={`flex items-center gap-3 p-3 font-headline font-bold uppercase tracking-tighter border-[3px] border-on-surface neo-interactive text-xs ${
                location.pathname === item.path
                  ? "bg-primary-container shadow-[2px_2px_0px_0px_rgba(48,52,44,1)]"
      ...
      <nav className="md:hidden sticky top-0 left-0 w-full bg-surface border-b-[3px] border-on-surface p-4 flex justify-between items-center z-50">
        <h1 className="text-2xl font-black text-on-surface italic tracking-tighter" onClick={() => navigate("/")}>GigB</h1>
        <div className="flex gap-2">
           <button onClick={handleLogout} className="p-2 border-[2px] border-on-surface bg-error-container neo-interactive">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-surface border-t-[3px] border-on-surface p-2 flex justify-around items-center z-50">
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`flex flex-col items-center p-2 rounded-sm ${location.pathname === item.path ? 'bg-primary-container border-[2px] border-on-surface shadow-[2px_2px_0px_0px_rgba(48,52,44,1)]' : ''}`}
          >
            <span className="material-symbols-outlined text-lg">{item.icon}</span>
