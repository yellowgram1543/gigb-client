...
// 10. HELPER UPLOADS COMPLETION PROOF
router.patch('/:id/completion-proof', protect, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Verify authorization: only the assigned helper can upload proof
    if (req.user._id.toString() !== task.helperId) {
      return res.status(403).json({ error: "Not authorized" });
    }

    // Only allow if task is in progress (ASSIGNED)
    if (task.status !== "ASSIGNED") {
      return res.status(400).json({ error: "Task not in progress" });
    }

    task.completionImageUrl = completionImageUrl;
    const updatedTask = await task.save();

    auditLog(`Completion proof uploaded for task #${id} by helper ${req.user._id}`);
    res.status(200).json(updatedTask);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
